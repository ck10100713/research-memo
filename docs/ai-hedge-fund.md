...
**AI Hedge Fund** 是 Virat Singh 開發的教育性概念驗證專案，模擬一個由多個 AI Agent 組成的虛擬對沖基金。系統讓 **13 位**傳奇投資人的 AI 分身從各自的投資哲學角度分析同一支股票，再經過 Valuation / Sentiment / Fundamentals / Technicals 四個分析 Agent 與 Risk Manager，最後由 Portfolio Manager Agent 整合意見做出最終倉位決策。

截至 2026-04-24，該專案擁有 **57,219 stars、9,953 forks、641 subscribers、112 open issues**，在過去約四週內 stars 成長 **+7,534**（從 2026-03-29 的 49,685），是 GitHub 最受關注的 AI 交易相關開源專案之一。**最新更新**：2026-04 新增第 13 位 **Nassim Taleb Agent**（黑天鵝／tail risk／antifragility 專家），讓總 Agent 數從 18 變 **19**。作者明確聲明這是 **純教育用途**，系統不會執行真實交易。

這個專案最大的價值不在於真正盈利，而在於它示範了「如何讓多個具有不同思維框架的 Agent 協同決策」——這個模式可以套用到任何需要多視角分析的領域。

## 核心功能

### 19 個 Agent 組成的決策鏈（2026-04 新增 Nassim Taleb）

```
[傳奇投資人 AI × 13]          [分析師 Agent × 4]
...
| **Nassim Taleb** *(新增)* | 黑天鵝、tail risk、反脆弱、不對稱報酬 |
...
| API 費用 | 19 個 Agent 同時呼叫 LLM，單次分析費用可觀 |
...
**5. 57K stars 背後的傳播機制**
這個專案的核心吸引力不是「能賺錢」，而是「有名字的 AI」——看到「Warren Buffett AI 分析了你的股票」比看到「AI 分析了你的股票」更有分享動機。**可識別的角色設定**是 AI 產品病毒傳播的有效策略。一個月內再加 7.5K stars、新增 Nassim Taleb Agent 馬上被社群討論，印證這個 pattern 的持續拉力——每加一個新傳奇人物，都是一次自帶話題的更新。

**6. 新增 Nassim Taleb 的訊號意義**
這個 commit 不只是「又加一個 agent」。其他 12 人都是價值 / 成長 / 宏觀路線，Taleb 是第一個**系統性做 tail-risk 與反脆弱**的 agent——等於在「好公司找到沒？」之外，補了「下行風險 / 黑天鵝」這條軸線。對應到真實 portfolio construction，這是從「選股模型」往「風險模型」擴張的關鍵一步。其他多 agent 系統要複製這個產品節奏，應該思考：**你的 agent roster 裡，是否也有「補足盲區」的角色？**
