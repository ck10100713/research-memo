# 研究更新

這頁是研究資料庫的更新索引，提供最新整理、按年份 / 月份、按主題三種入口。

> 日期以文章內標註的「建立日期 / 研究日期 / 發布日期」為主；若文章保留的是專案建立日，也依原文收錄。尚未標註日期的文章，統一收在下方的主題索引。

## 快速入口

- 最新整理：先看近期新增或整理完成的研究筆記。
- 按年份 / 月份：回看某一段時間集中研究過的主題。
- 按主題：直接從資料庫分類找到所有筆記。

## 最新整理

| 日期 | 類型 | 文章 |
| --- | --- | --- |
| 2026-03-27 | Coding Agent 工具 | [Agent Orchestrator](agent-orchestrator.md) |
| 2026-03-17 | Coding Agent 工具 | [Claude Cowork Dispatch](dispatch.md) |
| 2026-03-11 | Coding Agent 工具 | [gstack](gstack.md) |
| 2026-03-02 | AI Agent 框架 | [Paperclip](paperclip.md) |
| 2026-02-23 | Coding Agent 工具 | [Claude Financial Services Plugins](claude-financial-services-plugins.md) |
| 2026-02-09 | Coding Agent 工具 | [GitHub Copilot Configs](github-copilot-configs.md) |

## 按年份 / 月份

### 2026 年

#### 03 月

- `2026-03-27` [Agent Orchestrator](agent-orchestrator.md)  
  Composio 的多 agent 控制平面，核心是 issue、worktree、PR、CI、review 的自動閉環。
- `2026-03-17` [Claude Cowork Dispatch](dispatch.md)  
  用手機遠端遙控桌面 Claude Cowork，離開電腦也能派任務。
- `2026-03-11` [gstack](gstack.md)  
  Garry Tan 的 Claude Code 工作流系統，透過角色分工與 skill 串起完整開發 sprint。
- `2026-03-02` [Paperclip](paperclip.md)  
  開源 AI Agent 編排控制平面，重點在組織、治理、預算與任務追蹤。

#### 02 月

- `2026-02-23` [Claude Financial Services Plugins](claude-financial-services-plugins.md)  
  Anthropic 官方金融服務 Plugin，整合多種金融資料源與分析工作流。
- `2026-02-09` [GitHub Copilot Configs](github-copilot-configs.md)  
  GitHub Copilot 最佳設定集合，含 Agent、Instructions 與 Prompt 模板整理。
- `2026-02-04` [多 Agent 辯論會](multi-agent-debate.md)  
  使用 Copilot SDK 建構的多 Agent 辯論系統。
- `2026-02-03` [AutoGPT](autogpt.md)、[CrewAI](crewai.md)、[LangChain](langchain.md)、[GitHub Copilot CLI](copilot-cli.md)、[GitHub Copilot SDK](github-copilot-sdk.md)、[Copilot Ralph](copilot-ralph.md)、[Claude Code Showcase](claude-code-showcase.md)、[Learn Claude Code](learn-claude-code.md)、[MCP CLI](mcp-cli.md)、[OpenStock](openstock.md)、[StockStats](stockstats.md)、[TEJAPI Python Quant](tejapi_python_medium_quant.md)、[Insta-Booster](insta-booster.md)、[AI Agents（黃佳）](ai-agents.md)、[LLM Course](llm-course.md)、[Reference 快速參考](reference.md)

### 2024 年

#### 12 月

- `2024-12-28` [TradingAgents](tradingagents.md)  
  多 Agent 協作的量化交易決策系統。

### 2018 年

#### 12 月

- `2018-12-18` [Difftastic](difftastic.md)  
  結構化 diff 工具，用語法樹辨識真正有意義的程式碼變動。

## 按主題

### AI Agent 框架

[Autoresearch](autoresearch.md)、[AutoGPT](autogpt.md)、[CrewAI](crewai.md)、[LangChain](langchain.md)、[LangGraph State API](langgraph-state-api.md)、[LobeHub](lobehub.md)、[MiroFish](mirofish.md)、[Paperclip](paperclip.md)、[Page Agent](page-agent.md)、[Project Golem](project-golem.md)、[多 Agent 辯論會](multi-agent-debate.md)

### Coding Agent 工具

[App Store Preflight](app-store-preflight.md)、[Anthropic Harness Design](harness-design-long-running-apps.md)、[Agency Agents](agency-agents.md)、[Difftastic](difftastic.md)、[Claude HUD](claude-hud.md)、[Claude Financial Services Plugins](claude-financial-services-plugins.md)、[Claude Cowork Dispatch](dispatch.md)、[Everything Claude Code](everything-claude-code.md)、[Claude Code Showcase](claude-code-showcase.md)、[Claude Skills Guide](claude-skills-guide.md)、[Learn Claude Code](learn-claude-code.md)、[Lightpanda Browser](lightpanda-browser.md)、[GitHub Copilot CLI](copilot-cli.md)、[GitHub Copilot Configs](github-copilot-configs.md)、[GitHub Copilot SDK](github-copilot-sdk.md)、[Copilot Ralph](copilot-ralph.md)、[gstack](gstack.md)、[Agent Orchestrator](agent-orchestrator.md)、[MCP CLI](mcp-cli.md)、[OpenClaw Claude Proxy](openclaw-claude-proxy.md)、[Superpowers](superpowers.md)

### 量化交易

[OpenStock](openstock.md)、[StockStats](stockstats.md)、[TEJAPI Python Quant](tejapi_python_medium_quant.md)、[TradingAgents](tradingagents.md)

### 社群行銷

[Insta-Booster](insta-booster.md)

### AI 創作資源

[AI 圖像 Prompt Gallery 生態](ai-image-prompt-galleries.md)、[Uniform Map AI Prompts](uniform-map-prompts.md)

### 學習資源

[AI Agents（黃佳）](ai-agents.md)、[LLM Course](llm-course.md)、[Reference 快速參考](reference.md)
