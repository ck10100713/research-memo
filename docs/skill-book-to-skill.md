...
card_icon: "material-book-arrow-right-outline"
oneliner: "把任何技術書/文件（PDF、EPUB、DOCX…）轉成結構化 Claude Code skill，隨用隨查"
tags:
  - skills
  - knowledge-base
  - rag
...
| 作者 | virgiliojr94 |
| License | MIT |
| 規模 | 4.6k+ stars、578 forks（2026-06 數據） |

## Skill 概述

`book-to-skill` 是一個 Claude Code / Amp skill，用途是把**技術書或文件**（PDF、EPUB、DOCX、HTML、Markdown、純文字、RTF、MOBI/AZW）轉成一個**結構化的 agent skill**，安裝後可用 `/<書名-slug> <主題>` 隨用隨查，Claude 會載入對應章節並用書中內容回答。

它的核心主張是「**萃取結構，而非摘要**（Extract structure, not summaries）」：產物不是讀書報告，而是一套工具箱——具名框架（mental models）、可行動原則、技巧步驟、反模式（anti-patterns）、作者語氣校準。產生的 skill 採**分章按需載入**：`SKILL.md`（~4,000 tokens 核心心智模型 + 章節索引）常駐，各章節檔（~1,000 tokens）只在你問到該主題時才載入，不佔常駐預算。

作者特別把它與 RAG 對比：RAG 在**查詢時**做 chunk→embedding→相似度檢索（回答「跟你問題相近的片段」）；book-to-skill 在**編譯時**做一次深度分析，萃取作者實際建立的具名框架與適用時機（回答「這位作者建立的 12 個框架，可直接拿來推理」）。

## 安全審查結果

- **審查日期**：2026-06-08
- **檢查範圍**：全部 15 個原始碼檔案（`SKILL.md` + `scripts/extractor/` 全套 parser + `extract.py` + `dependencies.py` + CI workflow + `tools/validate_skill.py`），約 1,800 行
- **結果**：🟡 通過（含灰色地帶備註，經使用者核准收錄）
- **🔴 直接擋項目**：無。無資料外傳、無 base64/混淆執行、無 `curl|bash`、無憑證存取、無 `eval/exec/os.system/shell=True`、無 prompt injection。所有 `subprocess` 都用固定 argv、無 shell 注入，僅處理使用者指定的文件；CI workflow 為唯讀權限（`contents: read`）的標準 pytest/ruff
- **🟡 灰色地帶備註**（3 項，皆有妥善防護）：
    1. **pip 安裝套件**（`scripts/extractor/dependencies.py`）：會安裝 docling/PyPDF2/ebooklib 等，但清單是 `config.py` 寫死的固定 7 個知名套件、預設 `ask` 先徵詢同意、可用 `--no-install-missing` 關閉
    2. **呼叫外部工具**（`parsers/pdf.py`、`parsers/calibre.py`）：subprocess 執行 `pdftotext`/`pdfinfo`/`ebook-convert`，固定 argv、無字串插值
    3. **寫入家目錄**（`utils.py` + `SKILL.md`）：把產物寫入 `~/.claude/skills/<slug>/`——這正是工具的明確用途，由使用者主動觸發；Step 10 的 `shutil.rmtree` 只清理自己的 `/tmp/book_skill_work` 暫存目錄
...
| 模式 | 觸發 | 動作 |
|------|------|------|
| **1. Full Conversion**（預設） | 給文件/資料夾/glob 路徑、無特殊指示 | 跑完 Step 0–9，產出完整 skill |
| **2. Analyze Only** | 說「analyze」「先給我看」 | 只跑 Step 0–3，產出萃取報告後停手，不生成檔案 |
| **3. Generate from Analysis** | 已有先前的分析筆記 | 跳過 Step 0–3，用既有分析跑 Step 4–9 |
| **4. Update / Fold-in** | 指向既有 skill 資料夾/既存 slug | 擷取新內容後併入既有 skill 的章節/索引/詞彙表 |

### 主要步驟（Full Conversion）

```
Step 0   無參數 → 停手提示用法；解析 INPUT_PATHS 與選用的 skill slug
Step 1   驗證至少有一個支援格式的檔案（展開資料夾/glob）
Step 1.5 問使用者：內容是 technical（含程式碼/表格/公式）還是 text-heavy？
         technical → Docling（保留表格與程式碼，~1.5s/頁）
         text      → pdftotext → PyPDF2 → pdfminer（即時）
Step 2   跑 scripts/extract.py，輸出 /tmp/book_skill_work/{full_text.txt, metadata.json}
Step 2.5 讀 metadata 給「事前成本估算」（input/output token、預估費用、時間）後等使用者確認
Step 2.6 大書（>50k tokens）用 REPL 式存取：先 wc/grep/sed 定位章節，避免整檔讀入燒預算
Step 3   分析書本結構（標題、作者、章節、ToC、主題領域）
Step 4   問使用者目的（套用框架 / 用作者心智模型思考 / 查特定章節 / 全部）
Step 5   決定 skill 名稱（author-concept 或 by-title）與輸出位置；既存時問 更新/覆寫/改名
Step 6   建立目錄 <skill>/chapters/
Step 7   逐章生成摘要（每章 800–1,200 tokens：核心觀念、具名框架、心智模型、反模式、
         技術書另含 Code Examples 與 Reference Tables、Key Takeaways、Connects To）
Step 8   生成 glossary.md（詞彙）、patterns.md（技法/模式）、cheatsheet.md（決策表）
Step 9   生成 master SKILL.md（核心框架 < 4,000 tokens，章節索引 + 主題索引）
Step 10  清理 /tmp 暫存目錄並回報產出
```

### 萃取品質規則（Quality Rules）

1. 萃取結構，不做摘要——抓具名框架、精確表述、反模式，不寫章節流水帳
2. 保留作者的精確度——「The 5 Whys」≠「多問幾次為什麼」，保留原始命名
3. 密度優先於完整——1,000 token 的摘要勝過 10,000 token 的節錄
4. 實作者語氣——寫「Use X when Y」，不寫「書中解釋了 X」
5. 前置 SKILL.md——compaction 從尾端截斷，最重要的放最前
6. 章節檔按需載入——不問到就不佔預算
7. 絕不照抄原文——一律綜合、萃取訊號
8. 主題索引是關鍵——agent 靠它導航到正確章節檔

### 擷取效能基準（103 頁技術書，純 CPU）

| 方法 | 時間 | Tokens | 表格 | 程式碼區塊 |
|------|------|--------|------|-----------|
...
- **僅涵蓋書本內容**：產生的 skill 不含你 codebase 的實作脈絡，需與專案工具搭配
- **technical 模式慢**：Docling ~1.5s/頁，一本 400 頁書要數分鐘
- **MOBI/AZW 需外裝 Calibre**（`ebook-convert`，非 pip 套件）
- **萃取品質依賴來源格式**：掃描版 PDF 無內嵌文字時，本 skill 預設關閉 OCR（`do_ocr = False`），會擷取失敗
- **安全層面**（見上）：會 pip 安裝套件、呼叫外部工具、寫入 `~/.claude/skills/`，敏感環境請用 `--no-install-missing` 並留意

## 研究價值與啟示

### 關鍵洞察

1. **「編譯時萃取」vs「查詢時檢索」是這個 skill 最有價值的觀念**：它把 RAG 的相似度檢索，換成一次性的深度結構萃取——產物是「作者花數年建立的命名框架」，而非「跟查詢相近的句子」。這對知識庫設計是個重要分野：**結構化的少量 token 勝過可檢索的大量原文**。
2. **分章按需載入 = progressive disclosure 的具體實踐**：SKILL.md 常駐核心 + 章節檔延後載入，正是 Claude skill 系統 `navigation`/budget 思維的延伸。與本站 [research](skill-research.md) skill 把生成頁延後到 sync.py 重建、只手寫 `docs/<slug>.md` 是同一種「常駐輕、細節按需」哲學。
3. **成本透明化（Step 2.5）值得所有重量級 skill 借鏡**：在做任何生成前先估算 input/output token 與費用、等使用者確認——把「token 預算」當成一等公民，而非事後才發現燒太多。
4. **依賴處理的「ask + 固定清單 + 可關閉」是 🟡 行為的範本**：它需要 pip 安裝、呼叫外部工具（本來會是資安疑慮），但用「預設徵詢同意 + 寫死的套件白名單 + `--no-install-missing` 逃生口」把風險降到合理。這正是我們 [skill-review](skill-research.md) 流程中「灰色地帶可收錄」的好例子。
5. **它是「skill 生成 skill」的元工具**：本身是一個 agent skill，產物也是 agent skill——這類元層級工具（meta-skill）正在成為 agent 生態的基礎設施。

### 與其他專案的關聯

- [research（研究筆記產生器）](skill-research.md)：兩者都是「把非結構化來源轉成結構化知識資產」的工作流——research 產出網站筆記，book-to-skill 產出可載入的 skill；都採「常駐輕、細節按需」設計
- 本站「Coding Agent 工具」分類下大量 Claude skill 相關筆記（如 Claude Skills Guide、SkillOpt）：book-to-skill 是其中少數「自動生成 skill 內容」而非「教你寫 skill」的工具
- 概念上與 RAG 類專案（站內 RAG-Anything 等）形成對照：同樣面對「如何讓 LLM 用上一本書」，一個走檢索、一個走編譯期結構萃取

??? note "原文（English，完整 SKILL.md）"
